...
const getDashboard = async (req, res, next) => {
  try {
    ...
    // Optimize Dashboard using aggregations instead of memory
    // 1. Get recent tasks for UI (Due soon, Overdue, or Recently updated)
    const tasks = await prisma.task.findMany({
      where: {
      ...
      orderBy: { dueDate: 'asc' },
      take: 20 // Only send the top 20 tasks to dashboard view
    });
    ...
    // 2. Aggregate stats efficiently using DB count/groupBy
    const [totalTasks, overdueTasksCount, statusGroups, priorityGroups] = await Promise.all([
      prisma.task.count({
    ...
    const stats = {
      ...
      byStatus: statusGroups.reduce((acc, g) => ({ ...acc, [g.status]: g._count.status }), { TODO: 0, IN_PROGRESS: 0, IN_REVIEW: 0, DONE: 0 }),
      byPriority: priorityGroups.reduce((acc, g) => ({ ...acc, [g.priority]: g._count.priority }), { LOW: 0, MEDIUM: 0, HIGH: 0, URGENT: 0 })
    };
...
const getUserTasks = async (req, res, next) => {
  try {
    ...
    const tasks = await prisma.task.findMany({
      ...
      orderBy: { dueDate: 'asc' },
      take: 100 // Prevent fetching massive amounts
    });
...
module.exports = { create, getByProject, getById, update, updateStatus, remove, getDashboard, getUserTasks, addComment };
